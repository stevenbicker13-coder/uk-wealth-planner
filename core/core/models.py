...
class HouseholdInput:
    nation: str  # "England/Wales/Northern Ireland" or "Scotland"
    has_partner: bool
    children_count: int

    # Income
    you_salary_annual: float
    you_bonus_mode: str  # "None" | "Annual" | "Quarterly"
    you_bonus_amount: float  # annual if Annual, quarterly if Quarterly, else 0
    ...
    invest_existing: float
    invest_lump_sum: float
    invest_years: int

    # Childcare rows
    childcare_children: List[ChildCareChild]

    # Scenarios
    rate_rise_shock_pts: float
